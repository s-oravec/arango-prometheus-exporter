...
describe('makeGauge', () => {
    it('returns a gauge metric object', () => {
        ...
        const actual = metrics.makeOneDimensionalGauge(metric.name, metric.path, metric.help);
    ...
    it('format function returns correctly formated metric', () => {
        const metric = metrics.makeOneDimensionalGauge('metric', ['a', 'b'], '');
        const stats = {a: {b: 42}};
...
describe('formatAll', () => {
    it('returns the formatted values of all registered metrics', () => {
        metrics.registerOneDimensionalGauge('one_dimensional_gauge', ['a', 'b'], '')
        metrics.registerMultiDimensionalGauge({
            name: 'multi_dimensional_gauge',
            help: 'help',
            dimensions: [
                {name: "multi_dimensional_gauge{kind=\"a.b\"}", path: ["a", "b"]},
                {name: "multi_dimensional_gauge{kind=\"a.c\"}", path: ["a", "c"]}
            ]
        })
        ...
        const expected =
            '# HELP one_dimensional_gauge \n' +
            '# TYPE one_dimensional_gauge gauge\n' +
            'one_dimensional_gauge 42\n' +
            '# HELP multi_dimensional_gauge help\n' +
            '# TYPE multi_dimensional_gauge gauge\n' +
            'multi_dimensional_gauge{kind="a.b"} 42\n' +
            'multi_dimensional_gauge{kind="a.c"} 123\n' +
            '# HELP counter_value \n' +
